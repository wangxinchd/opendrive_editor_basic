...
OpenDriveParamPoly3 CubicPolynomial::ToOpenDriveParamPoly3(
...
double CubicPolynomial::CalculateArcLengthSimpson(
    double x1, double x2, int n) const {

    if (x2 <= x1 || n <= 0) return 0.0;
    if (n % 2 != 0) ++n;
...
double CubicPolynomial::CalculateArcLengthPolyline(
    double x1, double x2, int samples) const {

    if (x2 <= x1 || samples < 2) return 0.0;
...
CubicPolynomial CreateCubicHermite(
    double x1, double y1, double k1,
    ...
    const double a =
        (2.0 * (y1 - y2) + (k1 + k2) * h) / h3;
    const double b =
        (3.0 * (y2 - y1) - (2.0 * k1 + k2) * h) / h2;
    const double c = k1;
    const double d = y1;
