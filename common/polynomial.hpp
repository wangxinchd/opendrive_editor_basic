...
public:
  ...
  // --- Arc length ---
  [[nodiscard]] double CalculateArcLengthSimpson(double x1, double x2,
                                                 int num_intervals = 200) const;

  [[nodiscard]] double CalculateArcLengthPolyline(double x1, double x2,
                                                  int num_samples = 50) const;

  // --- Coefficients ---
  [[nodiscard]] std::array<double, 4> GetCoefficients() const;

  // OpenDRIVE
  [[nodiscard]] OpenDriveParamPoly3 ToOpenDriveParamPoly3(double x_start,
                                                          double x_end) const;
